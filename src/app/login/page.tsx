  ...
  return (
    <div className="min-h-screen bg-background text-on-background flex flex-col items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-sm mx-auto flex flex-col gap-6 bg-surface rounded-2xl border border-outline-variant p-6 sm:p-8 shadow-sm">
        ...
        {/* Error Alert */}
        {error && (
          <div className="p-3.5 bg-red-500/10 border border-red-500/30 rounded-xl text-xs text-red-600 dark:text-red-400 space-y-2.5">
            ...
            {error.includes('Google Sign-In') && (
              <div className="pt-2 border-t border-red-500/20 flex flex-col gap-2">
                ...
                >
                  Click here to set a password for email login
                </Link>
              </div>
            )}
            {error.toLowerCase().includes('confirm') && !error.includes('Google Sign-In') && (
              <button
                type="button"
