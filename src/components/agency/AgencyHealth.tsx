...
export const AgencyHealth: React.FC<AgencyHealthProps> = ({
  ...
  const meters = [
    { title: 'AI Receptionist Voice Calls', used: '14,280', cap: '30,000', icon: Bot, percent: 47 },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-1">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Client Health & Usage Center</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-xs space-y-1">
          ...
          <span className="text-3xl font-extrabold text-slate-900 font-mono">{healthyClients.length}</span>
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-xs space-y-1">
          ...
          <span className="text-3xl font-extrabold text-amber-600 font-mono">{attentionClients.length}</span>
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-xs space-y-1">
          ...
          <span className="text-3xl font-extrabold text-red-600 font-mono">{atRiskClients.length}</span>
      ...
      <section className="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs space-y-4">
        <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Aggregated Quota Usage</h3>
          ...
          {meters.map((m, idx) => (
            <div key={idx} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] font-mono text-slate-500">{m.used} / {m.cap}</p>
              </div>
      ...
      <section className="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs space-y-4">
        <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Recommended Agency Interventions</h3>
        <div className="space-y-3">
          <div className="p-4 bg-amber-50/60 border border-amber-200 rounded-xl flex items-center justify-between gap-4 text-xs">
            <div className="space-y-0.5">
              <span className="font-bold text-amber-900">Apex Comfort HVAC &bull; High AI Call Usage</span>
              <p className="text-amber-700">Client has consumed 88% of Starter plan AI receptionist allowance. Recommend upgrading to Professional.</p>
            </div>
            <button
              onClick={() => onManageClient && onManageClient(clients[0])}
              className="px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-600 text-white hover:bg-amber-700 shadow-2xs whitespace-nowrap"
            >
              Review Plan
            </button>
          </div>

          <div className="p-4 bg-red-50/60 border border-red-200 rounded-xl flex items-center justify-between gap-4 text-xs">
            <div className="space-y-0.5">
              <span className="font-bold text-red-900">ClearFlow Drain & Septic &bull; DNS Verification Incomplete</span>
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-bold bg-red-600 text-white hover:bg-red-700 shadow-2xs whitespace-nowrap"
            >
              Inspect DNS
            </button>
          </div>
